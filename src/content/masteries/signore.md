---
id: "1826ab78-ef1e-54fc-9d96-5431aaa0a803"
slug: "signore"
name: "Signore degli Inferi"
description: "Questa abilità oscura consente all'evocatore di soggiogare la morte animando i corpi dei defunti, che diventano quindi Inferius: servitori senza volontà al servizio del proprio padrone. Questi non morti non sono davvero tornati in vita , ma semplici marionette volte a eseguire il volere del loro creatore. Il processo di animazione richiede una concentrazione assoluta: l'evocatore non deve necessariamente recitare formule ad alta voce, ma focalizzarsi esclusivamente sulla propria magia e sul desiderio di infonderla nei corpi inerti. Durante l'attivazione, la terra attorno ai corpi inizia a fremere e l'aria a farsi gelida. Gli Inferi rispondono a questo richiamo emergendo violentemente dal terreno, dal fango o dalle ombre."
wisdom: "Oscura"
parameter: "Volontà"
image: "https://64.media.tumblr.com/tumblr_ln9b8eBhyE1qed6l4o1_500.gif"
values:
  - label: "Tipo"
    value: "Oscura"
  - label: "Parametro"
    value: "Volontà"
  - label: "Fatica"
    value: "Varia"
searchAliases:
  - "signore"
  - "signore degli inferi"
  - "maestria oscura"
status: published
migration:
  status: migrated
  sources:
    - document: "Maestrie"
      section: "Maestrie Oscure"
      item: "Signore degli Inferi"
---

<details><summary><strong>Specifiche</strong></summary>


**Requisiti**
Per utilizzare la Maestria il PG deve possedere almeno 20 Punti Sinistri.
Il requisito rappresenta la predisposizione morale necessaria a praticare una forma di necromanzia capace di profanare e rianimare i corpi dei defunti e usarli contro il prossimo.

**Raccolta dei Cadaveri**
Per garantire l'utilizzo della Maestria anche in contesti dinamici come Quest o Eventi, l'evocatore può accumulare cadaveri da rianimare in un secondo momento. Il reperimento avviene tramite una sessione di Ricerca Casuale.

**Ricerca Casuale di cadaveri**
Luoghi
La ricerca può essere effettuata esclusivamente in luoghi coerenti con la presenza di resti mortali, quali:
- Cimiteri e aree di sepoltura
- Fogne e sotterranei cittadini
- Foreste magiche o oscure
- Fondali marini e grotte isolate
- Luoghi disabitati o zone di conflitto
- Isola di Azkaban

Meccanica di Ritrovamento
A ogni post ambientato in uno dei suddetti luoghi, il PG può tentare il ritrovamento lanciando un dado. Il ritrovamento ha successo se l'esito è compreso tra 1 e Percezione + Sapienza Oscura/5:
- *Livello I:* 1d50
- *Livello II:* 1d40
- *Livello III:* 1d30
In caso di successo, si lancia *1d3* per determinare il numero di cadaveri rinvenuti.
È possibile effettuare più lanci di Ritrovamento nella stessa Role, ma il limite massimo di cadaveri complessivamente accumulabili per singola sessione di gioco è:
- *Livello I:* 3
- *Livello II:* 4
- *Livello III:* 5
Se un ritrovamento permetterebbe di superare tale limite, vengono acquisiti soltanto i cadaveri necessari a raggiungerlo.

Riserva Oscura e Deterioramento
I cadaveri rinvenuti entrano a far parte della Riserva Oscura del PG.
Il Player deve tenere traccia dei corpi accumulati nel proprio Taccuino, specificando il luogo ONGame in cui i resti vengono narrativamente conservati prima del trasporto.
*Trasporto:* per spostare e portare in Role i cadaveri è necessario il possesso di una Valigia Magica per Piante o Creature. È possibile trasportare un numero di cadaveri pari ai posti disponibili al suo interno, in quanto ciascun corpo occupa 1 slot equivalente a quello di un Non-Essere.
Cadaveri e Inferi già animati condividono la stessa capienza complessiva della Valigia e occupano entrambi 1 slot ciascuno.
La magia necrotica richiede tessuti non eccessivamente decomposti: i cadaveri non ancora animati hanno pertanto una durata di 1 anno OFFGame dalla data di ritrovamento, oltre la quale diventano inutilizzabili per la trasformazione in Inferius. Nel Taccuino è obbligatorio specificare la data OFFGame di raccolta di ogni corpo conservato.
Una volta trasformato in Inferius, il corpo non è più soggetto al limite di deterioramento previsto per i cadaveri non animati.

**Rilevazione e Rischio di Scoperta**
Raccogliere cadaveri costituisce un'Azione Illegale e deve pertanto essere gestito secondo l'apposito regolamento.
Il normale intervallo di scoperta è 1-30 su 1d100. Per ogni Cadavere o Inferius oltre i primi 3 complessivamente presenti nella Riserva Oscura, l'intervallo di scoperta aumenta di +1.
Cadaveri e Inferi vengono conteggiati insieme ai fini di questo limite.

**Animazione dei cadaveri**
L'Animazione trasforma un cadavere in un Inferius e richiede un'*Azione Principale*. Il PG deve avere con sé tutti i corpi che desidera animare e può effettuare una sola Animazione per post.
Una volta animato, l'Inferius viene conservato nella Riserva Oscura e nelle Role successive non deve essere animato nuovamente: per utilizzarlo sarà sufficiente richiamarlo attraverso l'apposita meccanica.

Inferi animabili con una singola Animazione
- *Livello I:* 1 Inferius
- *Livello II:* fino a 2 Inferi
- *Livello III:* fino a 3 Inferi

Forza degli Inferi
I Parametri posseduti dagli Inferi creati rispetto ai valori previsti dal Bestiario dipendono dal Livello della Maestria:
- *Livello I:* Parametri ridotti del 50%
- *Livello II:* Parametri ridotti del 25%
- *Livello III:* Parametri pieni previsti dal Bestiario
La riduzione riguarda i Parametri propri dell'Inferius indicati dal Bestiario e permane per tutta la sua esistenza.
Una volta completata l'Animazione, l'Inferius con i propri Parametri sostituisce il relativo Cadavere nella Valigia Magica e continua a occupare 1 slot.

Fatica Consumata per l'Animazione
Per ogni Inferius effettivamente animato, il PG consuma:
- *Livello I:* 2 Fatica
- *Livello II:* 1,5 Fatica
- *Livello III:* 1 Fatica
La Fatica viene calcolata sul numero complessivo di Inferi animati con la stessa Azione.

**Richiamare gli Inferi dalla Valigia**
OFFGame gli Inferi vengono custoditi nella Valigia Magica del PG. ONGame, invece, il Richiamo viene rappresentato narrativamente attraverso la loro comparsa dal terreno, dal fango, dalle ombre o da un altro punto coerente con la scena.
Per utilizzare il Richiamo il PG deve avere con sé la Valigia nella quale gli Inferi sono conservati e impiegare un'*Azione Principale*.

Il numero di Inferi richiamabili contemporaneamente attraverso la stessa Azione dipende dal Livello:
- *Livello I:* lancia 1d3 e può richiamare fino al numero di Inferi indicato dal risultato
- *Livello II:* fino a 3 Inferi
- *Livello III:* fino a 5 Inferi
Non è mai possibile richiamare più Inferi di quanti siano effettivamente conservati nella propria Riserva Oscura.

Fatica Consumata per il Richiamo
Per ogni utilizzo del Richiamo, indipendentemente dal numero di Inferi richiamati, il PG consuma:
- *Livello I:* 3 Fatica
- *Livello II:* 2 Fatica
- *Livello III:* 1 Fatica

**Mantenere il controllo**
Mantenere il controllo sugli Inferi permette al PG di indirizzarli verso un obiettivo specifico e, soprattutto, di evitare che si rivoltino contro il proprio creatore.
Per tutta la durata del controllo il PG deve dedicare parte della propria attenzione agli Inferi e non può compiere Azioni Principali. Può comunque compiere Azioni Bonus e Reazioni, parlare, osservare ciò che accade, effettuare il normale Movimento e svolgere azioni puramente narrative.

Il primo tiro di controllo viene effettuato nello stesso post in cui gli Inferi vengono richiamati e viene poi ripetuto a ogni successivo post dell'Evocatore nel quale desidera mantenerne il controllo.
Per ciascun Inferius controllato deve essere effettuato un tiro separato. Il controllo viene mantenuto se il PG estrae un risultato compreso fra 1 e Volontà + Sapienza Oscura/5 dopo aver lanciato:
- *Livello I:* 1d50
- *Livello II:* 1d40
- *Livello III:* 1d30

Se il tiro riesce, il PG mantiene il controllo di quell'Inferius fino al proprio post successivo.
Se il tiro fallisce, perde definitivamente il controllo di quell'Inferius per il resto della Role: esso torna ad agire secondo la propria natura e può considerare anche il suo precedente padrone come un normale bersaglio.
Il controllo di un Inferius perduto non può essere recuperato attraverso un nuovo tiro nei post successivi.
Il PG può inoltre decidere volontariamente di cessare il controllo di uno o più Inferi. Dal momento in cui lo fa, questi vengono considerati definitivamente fuori controllo per il resto della Role e non possono essere nuovamente soggiogati né recuperati nella Riserva Oscura.

Fatica Consumata per mantenere il controllo
Per ogni Inferius di cui il PG tenta di mantenere il controllo, a ogni proprio post consuma:
- *Livello I:* 1 Fatica
- *Livello II:* 0,5 Fatica
- *Livello III:* 0 Fatica
La Fatica viene consumata al momento del tentativo e viene quindi sottratta anche qualora il relativo tiro di controllo fallisca.
Se il PG non desidera o non può sostenere il costo necessario per uno degli Inferi richiamati, deve rinunciare al suo controllo: quell'Inferius viene immediatamente considerato fuori controllo.

**Ordini e Azioni degli Inferi**
Finché rimangono sotto controllo, gli Inferi possono essere indirizzati verso uno o più obiettivi e ricevono comandi semplici compatibili con la loro natura, come attaccare, inseguire, bloccare un passaggio o proteggere una determinata area.
Gli ordini possono essere impartiti o modificati contestualmente al mantenimento del controllo senza richiedere un'ulteriore Azione.
Ogni Inferius sotto controllo dispone di 1 propria Azione per ogni post dell'Evocatore, indipendente dalle Azioni del PG. Questa può essere utilizzata per compiere quanto consentito dalle caratteristiche e dalle capacità dell'Inferius previste dal Bestiario.
Le Azioni compiute dagli Inferi non consumano Azioni Principali, Azioni Bonus o Reazioni dell'Evocatore.
Gli Inferi non possono eseguire istruzioni complesse, elaborare strategie autonome o svolgere attività che richiedano ragionamento e capacità che non possiedono secondo il Bestiario.

**Recupero degli Inferi**
Gli Inferi superstiti ancora sotto il controllo del PG al termine della Role possono essere recuperati nella Riserva Oscura in base al Livello della Maestria:
- *Livello I:* gli Inferi superstiti non possono essere recuperati. Anche qualora il controllo sia stato mantenuto fino alla fine della Role, vengono definitivamente perduti.
- *Livello II:* per ogni Inferius superstite ancora sotto controllo si lancia 1d3. Con risultato 1-2, l'Inferius ritorna nella Riserva Oscura; con risultato 3 viene definitivamente perduto.
- *Livello III:* tutti gli Inferi superstiti ancora sotto controllo ritornano nella Riserva Oscura.

Un Inferius del quale il PG abbia perso o volontariamente ceduto il controllo non può mai essere recuperato nella Riserva Oscura.
Gli Inferi recuperati tornano a occupare normalmente il proprio slot nella Valigia Magica e mantengono i Parametri stabiliti al momento della loro Animazione.

**Distruzione degli Inferi**
Un Inferius distrutto durante la Role viene definitivamente eliminato. Il cadavere utilizzato per crearlo non può essere recuperato, rianimato o riutilizzato.

</details>
