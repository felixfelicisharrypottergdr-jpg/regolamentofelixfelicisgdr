...
async function createFelixIndex() {
  ...
  for (const entry of (await getCollection('schoolKnowledges')).filter((entry) => entry.data.status === 'published')) {
    index.set(entry.id, {
      felixId: entry.id,
      title: entry.data.name,
      type: 'schoolKnowledge',
      url: routeForStructured('schoolKnowledges', entry.data.slug),
    });
  }
